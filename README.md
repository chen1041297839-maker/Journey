...
- 当天路线用地图按圆周旅迹顺序画出来（Leaflet / OSM，坐标从导入的 GCJ-02 转成瓦片用的 WGS-84）。点名连锁店时，会推荐绕路最少的已有站，只挂成必买/店铺，**不会重排站点**
- 小七孔当天按 Xenia 的东进东出走法写进该站要点/机位（P2–P17）；晕车、枯水、东进东出说明挂在当天。大七孔游船和回贵阳烙锅只当晚备注，不改圆周旅迹站序、不往地图加园内点
- 版面：中文按完整句子排在卡片全宽里，不拆成一字一行；有配图才和图并排，没有配图时正文占满卡片。层级是标题 → 地点 → 要点 → 来源
